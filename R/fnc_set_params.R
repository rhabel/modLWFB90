...
#' @example inst/examples/fnc_set_params_ex.R
#' @export

fnc_set_params <- function(df.ids,
                          tree_species,
                          df.ind.info = NULL){
  ...
  if(!is.null(df.ind.info)){
    df.ind.info <- df.ind.info[order(df.ind.info$ID), ]
    if(!identical(df.ids$ID, df.ind.info$ID)){
