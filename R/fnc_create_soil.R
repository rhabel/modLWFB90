...
#' @param PTF the PTF to be used in the modeling process. Options are \code{HYPRES}, \code{PTFPUH2}, or \code{WESSOLEK}. Alternatively, if MvG parameters have been retrieved elsewhere (i.e. by lab analyses), \code{OWN_PARMS} can be selected to skip this.
...
fnc_create_soil <- function(df.ids,
  ...
  if(PTF == "OWN_PARMS"){
    ...
    cat(missingcol, "is missing in df.soils for PTF-application of ", PTF, "\n")
